...
st.title("候选人对比")
st.caption("批量上传简历与一份 JD，按匹配度排序（招聘视角）")
...
with col2:
    ...
    picked = st.multiselect(
        "已有简历", list(existing_map.keys()), label_visibility="collapsed"
    )

can_run = bool(jd and jd.strip()) and (bool(uploaded_files) or bool(picked))
if st.button(
    "批量匹配并排序", type="primary", disabled=not can_run, use_container_width=True
):
    resume_ids: list[str] = []
    id_to_label: dict[str, str] = {}

    if uploaded_files:
        with st.spinner("解析上传的简历..."):
    ...
    if not resume_ids:
        ...
        with st.spinner(f"对比中（{len(resume_ids)} 位候选人）..."):
