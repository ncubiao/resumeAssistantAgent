...
st.title("Agent 编排")
st.caption("LangGraph 多步编排：解析 → 分析 → 匹配/优化，附执行轨迹")
...
with col2:
    ...
    jd = st.text_area("目标岗位 JD（match / both 需要）", height=120)
    thread_id = st.text_input("thread_id（可选，复用同一会话累积 trace）")

if st.button(
    "运行 Agent 工作流", type="primary", disabled=not resume_id, use_container_width=True
):
    with st.spinner("Agent 工作流执行中..."):
    ...
    if isinstance(result, dict) and "error" in result:
        ...
        st.caption(f"模式: {result.get('mode')} · thread: {tid[:8]}")

        # ---- Trace 时间线（保留：作品集亮点） ----
        st.subheader("执行轨迹（Trace）")
        ...
        if result.get("optimize"):
            ...
            for i, sug in enumerate(result["optimize"].get("suggestions") or [], 1):
                with st.container(border=True):
                    st.markdown(
                        f"**{i}. {sug.get('category', '其他')}** {sug.get('improved', '')}"
                    )
