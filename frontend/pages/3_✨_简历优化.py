...
with tab1:
    ...
    if st.button("🚀 生成建议", type="primary", disabled=not resume_id):
        ...
        if isinstance(result, dict) and "error" in result:
            ...
            for i, sug in enumerate(suggestions, 1):
                with st.container(border=True):
                    ...
                    if sug.get("original"):
                        st.markdown(f"原文：`{sug['original']}`")
                    st.markdown(f"✏️ 改进：{sug.get('improved', '')}")
                    st.caption(f"💡 原因：{sug.get('reason', '')}")

with tab2:
    target_role = st.text_input("目标岗位（用于指导改写方向，可选）")
    paragraph = st.text_area("原始段落", height=180, placeholder="粘贴你想优化的段落...")

    if st.button("✍️ 重写", type="primary", disabled=not (paragraph and paragraph.strip())):
